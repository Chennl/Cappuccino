<?php
include_once '.models/StatementClass.php';
?>
 <html>
     <head>
	...
	<table id="dg" class="easyui-datagrid" title="Basic DataGrid"  
			data-options="singleSelect:true,collapsible:true,url:'./api/getpayments.php',method:'get',iconCls: 'icon-edit', singleSelect: true, fit:true,fitColumns:true,toolbar: '#tb', loadMsg:'正在查询...',	 
                        onClickCell: onClickCell, 
                        onBeforeEdit: onBeforeEdit,
                        onAfterEdit: onAfterEdit,
                        onCancelEdit: onCancelEdit,
                        onEndEdit: onEndEdit,
                        queryParams:{accountdate:'<?php echo date('y-m-01')?>',accounttype:2,description:'',category:''} ">
		<thead>
			<tr>
        ...
        <select id="accountdate" class="easyui-combobox" name="accountdate" style="width:100px;">      
             <?php       
		        ...
		        for($i=1;$i<13;$i++){
		         	 ...
		         	 echo "<option value='$str_date'> $str_date</option>";
		        }
	        ?> 
       </select>
        ...
        <a href="javascript:doSearch()" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true">搜索</a> 
     	...
     	<script type="text/javascript">
     	    ...
     	    function saveall() {
     	        ...
     	        $.post("/api/xjdpay", effectRow, function (rsp) {
     	            if (rsp.status) {
     	    ...
     	    function onEndEdit(index, row) {
     	        ...
     	        //    field: 'category'
     	        //});
     	        //row.category = $(ed.target).combobox('getText');
     	        editIndex = undefined;
     	        saveall();
     	    }
