<?php
	...
	if($action=='query'){
		...
		$query_where='where accounttype= '.$account_type.' and departmentid='.$department_id.' and accountdate between "'.$from.'" and "'.$to.'" ';
		...
		if(isset($_POST['insertdata'])){
			...
			if(!empty($updated)){
				...
				if($result){
					...
					echo json_encode(array('status'=>false,'errMsg'=>'failed to insert!'));
				}
				/* free result set */
				mysqli_free_result($result);
			}
